...
class StatusBarManager {
    ...
    displayExecutionFinished(uri, message) {
        const bar = this.getStatusBarForDoc(uri);
        if (bar) {
            bar.scriptExecutionStatus.text = message;
            bar.scriptExecutionStatus.tooltip = message;
            this.showAllItemsOfAStatusBar(uri);
        }
    }
    displayExecutionStarted(uri, message) {
        const bar = this.getStatusBarForDoc(uri);
        if (bar) {
            bar.scriptExecutionStatus.text = message;
            bar.scriptExecutionStatus.tooltip = message;
    ...
    displayConnectSuccess(uri, connAttributes, summaryInfo, currentSchema = undefined) {
        const bar = this.getStatusBarForDoc(uri);
        if (bar) {
            ...
            if (connAttributesVsCode) {
                displayText = helper.Utils.getConnectionLabel(connAttributesVsCode, currentSchema);
            }
            else if (summaryInfo) {
    ...
    displayConnectErrors(fileUri, connAttrs, response) {
        ...
        if (bar) {
            ...
            if (connAttrsVsCode) {
                connName = helper.Utils.getConnectionLabel(connAttrsVsCode);
            }
    ...
    displayConnectionProperties(uri, connNode, baseUri) {
        const bar = this.getStatusBarForDoc(uri);
        if (bar) {
            bar.connectionStatusDisplay.command = null;
            let displayText = baseUri + helper.Utils.getConnectionLabel(connNode.connectionProperties, connNode.schemaName);
            ...
            bar.connectionStatusDisplay.text = displayText;
            bar.connectionStatusDisplay.tooltip = (0, helper_1.getTooltipForConnection)(connNode.connectionProperties);
            this.showStatusBar(uri, bar.connectionStatusDisplay);
            let { text, tooltip } = this.prepareTNSAdminTextandTooltip(connNode.connectionProperties.tnsAdmin);
            bar.tnsAdminDisplay.text = text;
            bar.tnsAdminDisplay.tooltip = tooltip;
            this.showStatusBar(uri, bar.connectionStatusDisplay);
        }
    ...
    getLangServiceStatusText(connUri) {
        let statusText = "";
        if (connUri && intellisenseConnectionManager_1.IntellisenseConnectionManager.instance.intelliSenseSharingInfo.sharedConnections.has(connUri)) {
            let files = intellisenseConnectionManager_1.IntellisenseConnectionManager.instance.intelliSenseSharingInfo.sharedConnections.get(connUri);
            if (files) {
                let bar = undefined;
                for (let i = 0; i < files.length; i++) {
                    let fileUri = files[i];
                    bar = this.getStatusBarForDoc(fileUri);
                    if (bar) {
                        statusText = bar.languageServiceStatusDisplay.text;
                        if (statusText) {
                            break;
                        }
                    }
    ...
    displayLangServiceStatus(uri, message) {
        ...
        else if (intellisenseConnectionManager_1.IntellisenseConnectionManager.instance.intelliSenseSharingInfo.sharedConnections.has(uri)) {
            let files = intellisenseConnectionManager_1.IntellisenseConnectionManager.instance.intelliSenseSharingInfo.sharedConnections.get(uri);
            if (files) {
