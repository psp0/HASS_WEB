...
class FileStreamLogger {
    ...
    static async onConfigurationChanged(param) {
        const { enable, fileName, loglevel } = getLoggingConfig();
        if (!enable) {
    ...
    log(logData) {
        ...
        switch (logData.level) {
            ...
            case scriptExecutionModels_1.LogLevel.Warning:
                ...
                break;
            case scriptExecutionModels_1.LogLevel.Trace:
                FileStreamLogger.Instance.info(msg);
                break;
            default:
                break;
    ...
    info(message) {
        ...
        self.debug(method, message, scriptExecutionModels_1.LogLevel.Information);
    }
    warn(message) {
        const self = this;
        const method = self.logger ? self.logger.warn : undefined;
        self.debug(method, message, scriptExecutionModels_1.LogLevel.Warning);
    }
    error(message) {
        const self = this;
        const method = self.logger ? self.logger.error : undefined;
        self.debug(method, message, scriptExecutionModels_1.LogLevel.Error);
    }
...
class ChannelLogger {
    ...
    info(message) {
        this.debug(message, scriptExecutionModels_1.LogLevel.Information);
    }
    warn(message) {
        this.debug(message, scriptExecutionModels_1.LogLevel.Warning);
    }
    error(message) {
        this.debug(message, scriptExecutionModels_1.LogLevel.Error);
    }
    log(message) {
        this.debug(message, scriptExecutionModels_1.LogLevel.None);
    }
