...
class ResultDataServer {
    ...
    registerResultUI(panel, uri, executionId, parentEditor) {
        ...
        if (parentEditor !== undefined && parentEditor !== null) {
            clientDetails.parentEditor = parentEditor;
        }
    ...
    postConfigChangedToAll() {
        if (this.resultUIClientList && this.resultUIClientList.size > 0) {
            ...
            let logLevel = scriptExecutionModels_1.LogLevel[loglevelnumber];
            this.resultUIClientList.forEach((value, key) => {
                const clientInfo = value;
                const eventArgs = new scriptExecutionModels_1.ODTConfigurationChangedEvent();
                let querySettings = settings_1.Settings.getQueryExecutionSettingsForUri(clientInfo.ownerUri);
                eventArgs.logLevel = logLevel;
                eventArgs.ownerUri = "ConstantURI";
                eventArgs.dataBatchSize = querySettings.pageSize;
                eventArgs.csvDelimiter = querySettings.csvDelimiter;
                eventArgs.csvQualifier = querySettings.csvTextQualifier;
                eventArgs.rowNubmer = querySettings.rowNumber;
                eventArgs.cellTextLength = querySettings.cellTextLength;
                eventArgs.tooltipTextLength = querySettings.tooltipTextLength;
                const msg = {
                    type: scriptExecutionModels_1.MessageName.odtConfigChanged,
                    data: eventArgs,
                    executionId: null
                };
                if (clientInfo.panel && clientInfo.ready) {
    ...
    isClientRegister(ownerUri, executionId) {
        let results = this.getClientInfo(ownerUri, executionId);
