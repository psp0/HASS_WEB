...
class AutonomousDBInstance extends treeNodeBase_1.TreeNodeBase {
    ...
    async proceedWithOperation(operation, dbNodeName) {
        var ret = true;
    ...
    async openCreateConnectionUI(ociExplorerUIHandler, connectionCommandsHandler) {
        try {
            let response = await this.rootNode.getAccountComponent().ServicesClients.DatabaseServiceClient.getAutonomousDatabase({ autonomousDatabaseId: this.adbInstanceNodeProperties.adbInstanceID });
    ...
    async changeADBAdministratorPswd(param) {
        ...
        vscode.window.withProgress({
            ...
            var p = new Promise(async (resolve) => {
                try {
                    ...
                    promiseReturn.then(async (response) => {
    ...
    async startADBInstance(updateModel) {
        var proceed = await this.proceedWithOperation(localizedConstants_1.default.startOCIDatabaseConfirmation, this.adbInstanceNodeProperties.adbInstanceDisplayName);
        if (proceed) {
    ...
    async stopADBInstance(updateModel) {
        var proceed = await this.proceedWithOperation(localizedConstants_1.default.stopOCIDatabaseConfirmation, this.adbInstanceNodeProperties.adbInstanceDisplayName);
        var sucess = false;
        if (proceed) {
    ...
    async terminateADBInstance(updateModel) {
        var proceed = await this.proceedWithOperation(localizedConstants_1.default.deleteOCIDatabaseConfirmation, this.adbInstanceNodeProperties.adbInstanceDisplayName);
        var sucess = false;
        if (proceed) {
    ...
    async getADBNetworkAccessData() {
        let networkAccessData = null;
        try {
            let adb = await this.rootNode.getAccountComponent().ServicesClients.DatabaseServiceClient.getAutonomousDatabase({ autonomousDatabaseId: this.adbInstanceNodeProperties.adbInstanceID });
            networkAccessData = new autonomousDBModels_1.adbNetworkAccessData();
            networkAccessData.whitelistedIps = adb?.autonomousDatabase.whitelistedIps;
            networkAccessData.isAccessControlEnabled = adb?.autonomousDatabase.isAccessControlEnabled;
            networkAccessData.isMtlsConnectionRequired = adb?.autonomousDatabase.isMtlsConnectionRequired;
        }
        catch (error) {
            helper.logErroAfterValidating(error);
    ...
    async getMTLSAuthentication() {
        try {
            let adb = await this.rootNode.getAccountComponent().ServicesClients.DatabaseServiceClient.getAutonomousDatabase({ autonomousDatabaseId: this.adbInstanceNodeProperties.adbInstanceID });
            return [adb?.autonomousDatabase.isMtlsConnectionRequired, adb.autonomousDatabase.whitelistedIps];
        }
        catch (error) {
            throw error;
        }
