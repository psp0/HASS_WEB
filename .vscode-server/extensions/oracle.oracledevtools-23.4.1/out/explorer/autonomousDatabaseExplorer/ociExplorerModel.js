...
class ociProfileSettingManager {
    ...
    async deleteProfile(profileName) {
        ...
        try {
            await setup_1.Setup.getExtensionConfigSection().update(constants_1.Constants.compartmentNamePropertyName, this.profileProperties, true);
        }
        catch (error) {
            helper.logErroAfterValidating(error);
        }
    }
    async updateOCISetting(settingToUpdate, setting) {
...
class OCIExplorerModel {
    ...
    async updateProfileNodeCompartmentOrRegion(msg) {
    ...
    async launchOracleCloudSignUpURL() {
        try {
            ...
            await childProcess.exec(`${startCommand} ${constants_1.Constants.oracleSignUpCloudURL}`);
        }
        catch (error) {
            helper.logErroAfterValidating(error);
    ...
    async downloadWalletFile(walletFileRequest) {
        let profileNode = this.getProfileNode(walletFileRequest.profileName);
        if (profileNode) {
            profileNode.downloadWalletFile(walletFileRequest);
        }
    }
    async fetchCompartmentList(ociNode) {
        if (ociNode) {
            await ociNode.launchCompartmentAndRegionUIList();
        }
    }
    async LaunchCreateNewDatabaseUI(ociNode) {
        if (ociNode) {
            await ociNode.LaunchCreateNewDatabaseUI();
        }
    }
    updateProfileNode(rootNode) {
        this.modelChanged.emit(this.MODEL_CHANGED, rootNode);
    }
    deleteProfileNode(profilName) {
        this.profileNodesMap.delete(profilName);
        this.modelChanged.emit(this.MODEL_CHANGED);
    }
    getConfigFilePath() {
        return this.configurationFilePath;
    }
    getConfigFileDataFromConfigFile(profileName) {
    ...
    async initializeModel(compareExistingProfile) {
        let profileName = "";
        let profileNode = null;
