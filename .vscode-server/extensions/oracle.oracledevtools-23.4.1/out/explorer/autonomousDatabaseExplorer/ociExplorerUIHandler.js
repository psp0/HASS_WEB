...
class OCIExplorerUIHandler {
    constructor(adbExplorerModel) {
        ...
        resultsDataServer_1.ResultDataServer.instanceSingle.addMessageHandler(scriptExecutionModels_1.MessageName.ociLaunchChangeadminPswdDialogRequestMessage, (message) => {
            this.launchAdminPwdDialogRequest(message);
        });
    ...
    async sendCompartmentList(message) {
        try {
            let profileNode = this.adbExplorerModel.getProfileNode(message.profileName);
            if (profileNode != null) {
                var compartmentResponse = await profileNode.getCompartmentResponse();
    ...
    async sendCreateNewDBInializationData(message) {
        let request = message;
        ...
        let autonoumousDatabaseInfo = await adbWorkLoadNode.getCreateAutonomousDBInitializationData();
        ...
        createADBInitializationData.autoScalingForStorage = autonoumousDatabaseInfo.autoScalingForStorage;
        ...
        createADBInitializationData.computeModel = autonoumousDatabaseInfo.computeModel;
        createADBInitializationData.storageLabel = autonoumousDatabaseInfo.storageLabel;
        createADBInitializationData.storageUnit = autonoumousDatabaseInfo.storageUnit;
    ...
    async updateAutonomousDBName(message) {
    ...
    async sendCreateNewAutonomousDBResponse(message) {
    ...
    async sendAutonomousContainerDatabases(message) {
    ...
    launchAdminPwdDialogRequest(message) {
    ...
    updateCompartmentAndRegion(message) {
        ...
        let errorMessage = "";
        vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: progressMsg }, async (progress, token) => {
                ...
                try {
                    switch (msg.updateType) {
                        case autonomousDBModels_1.CompartmentAndRegionUpdateType.Region:
                            try {
                                canProceedWithUpdate = await this.adbExplorerModel.updateProfileNodeCompartmentOrRegion(msg);
    ...
    async openDownloadCredentialsFileUI(connectionCommandsHandler, databaseInfo) {
        try {
            let explorerModel = ociExplorerModel_1.OCIExplorerModel.getInstance();
            let args = helper_1.Utils.getConnectionUIArguments("", false, databaseInfo, (++explorerModel.executionID).toString());
            helper_1.Utils.openWebviewPanel(args, "OracleDownloadCredentialsFile", "DownloadCredentialsFile page is active and visible", ociExplorerManager_1.OCIExplorerManager.Instance.vsCodeConnector);
        }
        catch (error) {
            helper.logErroAfterValidating(error);
        }
    }
    async openCreateConnectionUI(connectionCommandsHandler, databaseInfo) {
        try {
            let explorerModel = ociExplorerModel_1.OCIExplorerModel.getInstance();
            let args = helper_1.Utils.getConnectionUIArguments("", false, databaseInfo, (++explorerModel.executionID).toString());
            helper_1.Utils.openWebviewPanel(args, "OracleADBConnectionManagement", "ADBConnection page is active and visible", ociExplorerManager_1.OCIExplorerManager.Instance.vsCodeConnector);
        }
        catch (error) {
            helper.logErroAfterValidating(error);
        }
    }
    async openCreateADBDatabaseUI(workloadNode) {
        try {
            ...
            this.openOCIUI(args, "OracleCreateADBDatabaseModule", existingPanel, true, (uri, execId) => {
                ...
                let clientRegister = resultsDataServer_1.ResultDataServer.instanceSingle.isClientRegister(uri, execId);
                if (clientRegister) {
    ...
    openNewOCIPage(args, panelType) {
        try {
            ...
            panel.webview.html = scriptExcutionCommandHandler_1.ScriptExecutionCommandHandler.generateHtml(ociExplorerManager_1.OCIExplorerManager.Instance.vsCodeConnector, args, panel);
            panel.onDidDispose(() => {
    ...
    openOCIUI(args, viewType, existingPanel, refreshExistingPanel, uiSpecificLaunchHandler = null, uiSpecificDisposeHandler = null) {
        try {
            ...
            if (!panel) {
                ...
                panel.title = args.windowTitle;
                panel.onDidDispose(() => {
                    uiSpecificDisposeHandler?.(args.windowUri, args.executionId);
                }, undefined, ociExplorerManager_1.OCIExplorerManager.Instance.vsCodeConnector.ExtensionContext.subscriptions);
                panel.onDidChangeViewState((e) => {
                    if (e && e.webviewPanel) {
                        if (e.webviewPanel.active && e.webviewPanel.visible) {
                ...
                uiSpecificLaunchHandler?.(args.windowUri, args.executionId);
            }
    ...
    async launchChangeADBAdministratorPswdUI(execID, profileName, adbDisplayName, adbDatabaseID, compartmentFullPath) {
        let args = new scriptExcutionCommandHandler_1.RootWebPageArguments();
    ...
    async sendADBConnectionStringsData(message) {
        try {
            ...
            if (dbNode.canGetConnectionString()) {
                connectionStringsResponse = await dbNode.getADBConnectionstrings();
            }
            else {
    ...
    async getOCIVCNList(msg) {
        ...
        try {
            ...
            vcnListResponse.vcnData = await autonomousDBUtils_1.AutonomousDBUtils.getVCNList(profileNode.getAccountComponent().AuthProvider, compartmentID);
    ...
    async getInitializeNetworkAccessTypeData(msg) {
        let dbNode = null;
        try {
            let message = msg;
            let profileNode = this.adbExplorerModel.getProfileNode(message.profileName);
            dbNode = profileNode.getAutonomousDBNode(message.adbDatabaseID);
            let networkAccessData = await dbNode.getADBNetworkAccessData();
    ...
    async getInitializeMutualAuthenticationData(msg) {
        let dbNode = null;
        try {
            let message = msg;
            let profileNode = this.adbExplorerModel.getProfileNode(message.profileName);
            dbNode = profileNode.getAutonomousDBNode(message.adbDatabaseID);
            let mtlsConnectionRequired = await dbNode?.getMTLSAuthentication();
    ...
    async getPublicIPAddressRequest(msg) {
        try {
            let request = msg;
            var response = await autonomousDBUtils_1.AutonomousDBUtils.getPublicIPAddress({ windowUri: request.windowUri });
    ...
    async openGetADBConnectionStringsUI(connectionCommandsHandler, databaseInfo) {
        try {
            let explorerModel = ociExplorerModel_1.OCIExplorerModel.getInstance();
            let args = helper_1.Utils.getConnectionUIArguments("", false, databaseInfo, (++explorerModel.executionID).toString());
            helper_1.Utils.openWebviewPanel(args, "OracleADBConnectionManagement", "ADBConnection page is active and visible", ociExplorerManager_1.OCIExplorerManager.Instance.vsCodeConnector);
    ...
    async launchGetADBConnectionStringsUI(execID, profileName, adbDisplayName, adbDatabaseID, compartmentFullPath, adbworkloadType) {
        let args = new scriptExcutionCommandHandler_1.RootWebPageArguments();
        ...
        helper_1.Utils.openWebviewPanel(args, "ociGetADBConnectionStringsModule", "getADBConnectionStringsUI page is active and visible", ociExplorerManager_1.OCIExplorerManager.Instance.vsCodeConnector);
    }
    async launchConfigureWalletlessConnectivityAndNetworkAccessUI(execID, dbInfo, compartmentName, compartmentFullPath, adbworkloadType) {
        let args = new scriptExcutionCommandHandler_1.RootWebPageArguments();
        ...
        args.adbWorkLoadType = adbworkloadType;
        helper_1.Utils.openWebviewPanel(args, "adbConfigureWalletlessConnectivityAndNetworkAccessModule", "getadbConfigureWalletlessConnectivityAndNetworkAccessModuleUI page is active and visible", ociExplorerManager_1.OCIExplorerManager.Instance.vsCodeConnector);
    }
}
