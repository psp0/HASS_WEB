...
class QueryBookmarkManager {
    constructor(connectionCommandsHandler, vsCodeConnector, scriptExecutor, prompter, untitledDcoumentProvider, fileLogger) {
        ...
        vscode.commands.registerCommand(constants_1.Constants.moveUpBookmarkQueryItem, async (bookMarkNode) => {
            try {
        ...
        vscode.commands.registerCommand(constants_1.Constants.movedownBookmarkQueryItem, async (bookMarkNode) => {
            try {
        ...
        vscode.commands.registerCommand(constants_1.Constants.moveUpBookmarkQueryGroup, async (bookMarkNode) => {
            try {
        ...
        vscode.commands.registerCommand(constants_1.Constants.movedownBookmarkQueryGroup, async (bookMarkNode) => {
            try {
        ...
        vscode.commands.registerCommand(constants_1.Constants.addBookmarkQuery, async () => {
            try {
        ...
        vscode.commands.registerCommand(constants_1.Constants.editBookmarkQuery, async (queryBookmarkNode) => {
            try {
        ...
        vscode.commands.registerCommand(constants_1.Constants.selectBookmarkQuery, async (node) => {
            try {
        ...
        vscode.commands.registerCommand(constants_1.Constants.runBookmarkQuery, async (queryBookmarkNode) => {
            try {
        ...
        vscode.commands.registerCommand(constants_1.Constants.deleteBookmarkQuery, async (queryBookmarkNode) => {
            try {
        ...
        vscode.commands.registerCommand(constants_1.Constants.clearallBookmarks, async () => {
            try {
        ...
        vscode.commands.registerCommand(constants_1.Constants.openBookmarkQuery, async (queryBookmarkNode) => {
            try {
        ...
        vscode.commands.registerCommand(constants_1.Constants.renameBookmarkQuery, async (queryBookmarkNode) => {
            try {
        ...
        vscode.commands.registerCommand(constants_1.Constants.addBookmarkFolder, async () => {
            try {
    ...
    async addBookmarkItem() {
        try {
            var selected = this.vsCodeConnector.getActiveDocumentSelectedText();
            if (selected == undefined || selected.length == 0) {
                var queryInfo = await utilities_1.QueryBookmarkUtil.BookmarkQuery(utilities_1.QueryBookmarkUtil.ConstructParams(this.vsCodeConnector));
    ...
    async addQueryToBookmarks(sqlquery) {
        try {
            var groupName;
            var queryName;
            const questions = [];
    ...
    async addBookmarkHelper(query, groupName, queryName) {
        var modelUpdated = await this.updateModel({
    ...
    async runBookmarkItem(node) {
        try {
            var response = await utilities_1.QueryBookmarkUtil.ConnectFirstSQLInBookmarkQuery(new queryBookmarkRequest_1.detectConnectAsStartSQLParams(node.query, constants_1.Constants.bookmarkOwnerIdentifier));
            if (response.connectFirstSQL) {
                utilities_1.QueryBookmarkUtil.RunBookmarkItem(node, this.scriptExecutor, QueryBookmarkManager.ownerIdentifier, response.connectFirstSQL);
            }
            else {
                var conProps = await this.connectionCommandsHandler.showConnections();
                if (conProps != undefined) {
                    var connected = await this.connectionCommandsHandler.connect(QueryBookmarkManager.ownerIdentifier, conProps, false, false, connectionRequest_1.ConnectionSource.Bookmarks);
                    if (connected) {
                        utilities_1.QueryBookmarkUtil.RunBookmarkItem(node, this.scriptExecutor, QueryBookmarkManager.ownerIdentifier, response.connectFirstSQL);
                    }
                }
    ...
    async updateModel(params) {
        try {
            if (this.queryBookmarkDataModel.changeDetected()) {
                ...
                const proceedOverwrite = await this.prompter.promptSingle(question);
