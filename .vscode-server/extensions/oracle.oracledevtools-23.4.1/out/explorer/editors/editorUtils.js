...
class editorUtils {
    static async refreshEditorContents() {
        await vscode.commands.executeCommand('workbench.action.files.revert');
    }
    static async openEditor(editorUri, connectionNode) {
        try {
            let systemManager = (0, extension_1.getSystemManager)();
            systemManager.codeEditorProvider.lastOpenedUri = undefined;
            let document = await vscode.workspace.openTextDocument(editorUri);
            if (document) {
    ...
    static getQueryParams(uriQuery) {
        let params = null;
        if (uriQuery) {
            var regex = new RegExp("sessionId=(?<sessionId>.*)&connectionUri=(?<connectionUri>.*)&schemaname=(?<schemaname>.*)&objectname=(?<objectname>(.|\n|\r\n)*)&objectType=(?<ddexObjectType>.*)&connectionname=(?<connectionname>.*)");
    ...
    static async downloadEditorScript(uri, dataExpManger, vsCodeConnector, connectionCommandsHandler) {
        try {
            ...
            let params = editorUtils.getQueryParameters(uri);
            if (params) {
                const [sourceText, createdDateTime, modifiedDateTime, requestResponse, connOpen] = await utilities_1.ExplorerUtilities.getCodeObjectSource(params.connectionUri, params.ddexObjectType, params.objectname, params.schemaname, true);
                ...
                if (requestResponse.messageType === dataExplorerRequests_1.DataExplorerFetchMessageType.Error) {
                    ...
                    try {
                        ...
                        if (config) {
                            ...
                            let fileToSave = await vscode.window.showSaveDialog(saveOptions);
    ...
    static getCodeEditorUri(treeNode, ddexObjectType, dataExpManger) {
        ...
        try {
            if (treeNode) {
                ...
                editorUri = this.getEditorUri(constants_1.Constants.oracleScheme, connectionNode, dbObjectTye, treeNode.schemaName, treeNode.objectName, treeNode.connectionURI, ddexObjectType);
            }
        }
        catch (error) {
    ...
    static getEditorUri(docScheme, connNode, dbObjectTye, schemaName, objectName, connectionURI, ddexObjectType) {
        let tmpConnectionName = this.removeInvalidCharsFromFilename(connNode.connectionProperties.name);
        ...
        let uriQuery = editorUtils.getUriQueryString(connNode.connectionProperties.name, schemaName, objectName, connectionURI, ddexObjectType);
        let file = `${docScheme}:///${tmpConnectionName}/${tmpDbObjectTye}/${tmpSchemaName}/${tmpObjectName}?${uriQuery}`;
    ...
    static async updateExplorerFileOnConnectionRename(newConnNode) {
        try {
            ...
            for (const uri of openUris) {
                ...
                if (editorUri && editorUri.query) {
                    ...
                    if (params && params.connectionUri && params.connectionUri === newConnNode.connectionURI) {
                        ...
                        let newUri = editorUtils.getEditorUri(constants_1.Constants.oracleScheme, newConnNode, editorUtils.getObjectTypeFromDdexType(params.ddexObjectType), params.schemaname, params.objectname, params.connectionUri, params.ddexObjectType);
                        await edit.renameFile(oldUri, newUri);
                        await vscode.workspace.applyEdit(edit);
                        systemManager.statusController.displayConnectionProperties(newUri.toString(), newConnNode, utilities_1.TreeViewConstants.baseUri);
                    }
                }
            }
        }
