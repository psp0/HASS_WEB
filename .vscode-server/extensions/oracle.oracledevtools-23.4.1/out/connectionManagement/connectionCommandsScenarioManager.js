...
class ConnectionPropertiesPresenter {
    static createNameValueConnectionProperties(connectionProp) {
        ...
        connPropertiesKeyValue.attributes[constants_1.ConnectionPropKeys.connectionType] = connectionProp.connectionType;
        ...
        connPropertiesKeyValue.attributes[constants_1.ConnectionPropKeys.useConnectionCredsFromWalletFile] = connectionProp.useConnectionCredsFromWalletFile;
        ...
        connPropertiesKeyValue.attributes[constants_1.ConnectionPropKeys.useCompatibleNamesDirectoryPath] = connectionProp.useCompatibleNamesDirectoryPath;
        connPropertiesKeyValue.attributes[constants_1.ConnectionPropKeys.filters] = connectionProp.filters;
        connPropertiesKeyValue.attributes[constants_1.ConnectionPropKeys.defaultAIProfile] = connectionProp.defaultAIProfile;
        connPropertiesKeyValue.attributes[constants_1.ConnectionPropKeys.labelStore] = connectionProp.passwordStore;
        let cp = connectionProp;
        if (cp) {
            if (cp.databaseHostName)
                connPropertiesKeyValue.attributes[constants_1.ConnectionPropKeys.databaseHostName] = cp.databaseHostName;
            if (cp.databasePortNumber)
                connPropertiesKeyValue.attributes[constants_1.ConnectionPropKeys.databasePortNumber] = Number.parseInt(cp.databasePortNumber);
            if (cp.databaseServiceName)
                connPropertiesKeyValue.attributes[constants_1.ConnectionPropKeys.databaseServiceName] = cp.databaseServiceName;
        }
        return connPropertiesKeyValue;
    }
    static async promptsAndAcceptsVariousConnProperties(connProperties, isPasswordRequired, wasPasswordEmptyInConfigFile, prompter, connRepository, defaultVSCodeProfileValues, deleteAndSaveProfile = false, ignoreFocusOut = false) {
        let connPropSaved = (connProperties);
        try {
            const questions = await ConnectionPropertiesPresenter.getConnectionPropertiesFromUser(connProperties, defaultVSCodeProfileValues);
            const answers = await prompter.prompt(questions, ignoreFocusOut);
            if (!answers) {
                connPropSaved = null;
            }
        }
    ...
    static async getConnectionPropertiesFromUser(connectionProp, defaultConnectionroperties, profileNew, profileNameQs) {
