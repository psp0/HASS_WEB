...
<div class="modal-grid">
    ...
    <div class="section" id="section-b">
        ...
        <?php if ($visitData): ?>
            <table>
                <?php if ($requestStatus === '대기중'): ?>
                    <?php if (count($visitData) == 2): ?>
                        <tr><th rowspan=2>선호 날짜</th><td><?= htmlspecialchars($visitData[0]['PREFER_DATE']) ?></td></tr>
                        <tr><td><?= htmlspecialchars($visitData[1]['PREFER_DATE']) ?></td></tr>
                    <?php else: ?>
                        <tr><th>선호 날짜</th><td><?= htmlspecialchars($visitData[0]['PREFER_DATE']) ?></td></tr>
                    <?php endif; ?>
                <?php else: ?>
                    <?php foreach ($visitData as $data): ?>
                        <tr><th>방문 일자</th><td><?= htmlspecialchars($data['VISIT_DATE']) ?></td></tr>
                        <tr><th>방문예정 확정일</th><td><?= htmlspecialchars($data['DATE_CREATED']) ?></td></tr>
                    <?php endforeach; ?>
                <?php endif; ?>
            </table>
        <?php else: ?>
