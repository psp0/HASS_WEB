...
<div class="modal-grid">
    ...
    <?php if ($visitData): ?>
        <table>
            <?php
            // If the request status is '대기중' (pending), show preferred date
            if ($requestStatus === '대기중') {
                if (count($visitData) > 1) {
                    echo "<tr><th rowspan='" . count($visitData) . "'>선호 날짜</th>";
                    foreach ($visitData as $index => $dateRow) {
                        if ($index > 0) echo "<tr>"; // Start a new row for each additional date
                        echo "<td>" . htmlspecialchars($dateRow['PREFER_DATE']) . "</td></tr>";
                    }
                } else {
                    echo "<tr><th>선호 날짜</th><td>" . htmlspecialchars($visitData[0]['PREFER_DATE']) . "</td></tr>";
                }
            } 
            // Otherwise, show visit date and confirmed date
            else {
                if (isset($visitData['VISIT_DATE']) && isset($visitData['DATE_CREATED'])) {
                    echo "<tr><th>방문 일자</th><td>" . htmlspecialchars($visitData['VISIT_DATE']) . "</td></tr>";
                    echo "<tr><th>방문 확정일</th><td>" . htmlspecialchars($visitData['DATE_CREATED']) . "</td></tr>";
                } else {
                    echo "<tr><td colspan='2'>방문 정보가 없습니다.</td></tr>";
                }
            }
            ?>
        </table>
    ...
    <div class="section" id="section-d">
        ...
        <?php if ($workerData): ?>
            <table>
                <tr><th>기사 ID</th><td><?= htmlspecialchars($workerData['WORKER_ID']) ?></td></tr>
                <tr><th>기사 이름</th><td><?= htmlspecialchars($workerData['WORKER_NAME']) ?></td></tr>
            </table>
        <?php else: ?>
            <p>기사 정보가 없습니다.</p>
        <?php endif; ?>
