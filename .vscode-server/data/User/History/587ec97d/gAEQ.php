...
function showModal(requestId,requestStatus) {
    // Fetch request details using AJAX
    fetchRequestDetails(requestId,requestStatus);
...
function fetchRequestDetails(requestId, requestStatus) {
    ...
    // 요청 본문에 두 파라미터를 함께 보내기
    var params = "requestId=" + encodeURIComponent(requestId) + "&requestStatus=" + encodeURIComponent(requestStatus);
    xhr.send(params);
}
...
<div class="data-container">
    ...
    while ($rowA = oci_fetch_assoc($stmtA)) {
        echo "<tr onclick='showModal(" . htmlspecialchars($rowA['REQUEST_ID']) . ", \"" . htmlspecialchars($rowA['REQUEST_STATUS']) . "\")'>";
    ...
    while ($rowB = oci_fetch_assoc($stmtB)) {
        echo "<tr onclick='showModal(" . htmlspecialchars($rowB['REQUEST_ID']) . ", \"" . htmlspecialchars($rowB['REQUEST_STATUS']) . "\")'>";
