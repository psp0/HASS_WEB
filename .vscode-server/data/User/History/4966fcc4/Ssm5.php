...
if (!empty($_GET['coverageArea'])) {
    ...
    foreach ($_GET['coverageArea'] as $area) {
        if ($area == 'above100')
            $coverageConditions[] = "mas.COVERAGE_AREA >= 100";
        elseif ($area == '88-99')
            $coverageConditions[] = "mas.COVERAGE_AREA BETWEEN 88 AND 99";
        elseif ($area == '51-82')
            $coverageConditions[] = "mas.COVERAGE_AREA BETWEEN 51 AND 82";
        elseif ($area == 'below50')
            $coverageConditions[] = "mas.COVERAGE_AREA <= 50";
    }
...
// 결과가 있는지 확인
$html = '';
$resultFound = false; // 결과 확인을 위한 플래그
while ($row = oci_fetch_array($stmt, OCI_ASSOC + OCI_RETURN_NULLS)) {
    $resultFound = true; // 결과가 있음을 표시
...
// 결과가 없을 경우 메시지 설정
if (!$resultFound) {
    $html = "<p>No matching data found.</p>";
}
