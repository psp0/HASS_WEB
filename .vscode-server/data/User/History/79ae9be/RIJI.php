    ...
    <div class="filter-panel">
      ...
      <div class="panel" style="max-height: 1000px;">
        <div class="checkbox-group">
          <valu><input type="checkbox" value="flat">판형</label>
...
<script>
  ...
  subscriptionSlider.noUiSlider.on('set', function (values) {
    ...
    if (existingTag) {
      // 기존의 가격 필터 태그가 있으면 업데이트
      existingTag.querySelector('span').textContent = priceRange;
    } else {
      // 새로운 가격 필터 태그 생성
      var filterTag = document.createElement('div');
      filterTag.className = 'filter-tag';
      filterTag.setAttribute('data-type', 'price');
      filterTag.innerHTML = `<span>${priceRange}</span><span class='remove-filter'>&times;</span>`;
      // '×' 버튼을 클릭했을 때 필터 제거 이벤트 추가
      filterTag.querySelector('.remove-filter').addEventListener('click', function () {
        subscriptionSlider.noUiSlider.set([10000, 100000]); // 슬라이더 초기화
        filterContainer.removeChild(filterTag); // 필터 태그 제거
      });
      filterContainer.appendChild(filterTag);
    }

    filterModels(); // 필터링 함수 호출
  });


  // 체크박스 상태 변경에 따른 이벤트 리스너 추가
  document.querySelectorAll('.checkbox-group input[type="checkbox"]').forEach(function (checkbox) {
    checkbox.addEventListener('change', function () {
      var filterContainer = document.getElementById('selected-filters');
      var filterValue = this.nextSibling.textContent.trim();

      if (this.checked) {
        // 체크박스가 선택되었을 때 필터 태그 생성
        var filterTag = document.createElement('div');
        filterTag.className = 'filter-tag';
        filterTag.innerHTML = `<span>${filterValue}</span><span class='remove-filter'>&times;</span>`;
        filterTag.querySelector('.remove-filter').addEventListener('click', function () {
        ...
        // 체크박스가 해제되었을 때 해당 필터 태그 제거
        Array.from(filterContainer.children).forEach(function (tag) {
          if (tag.querySelector('span').textContent === filterValue) {
  ...
  document.getElementById('reset-filters').addEventListener('click', function () {
    ...
    var filterContainer = document.getElementById('selected-filters');
    // 모든 필터 태그 제거 (라벨과 초기화 버튼은 제외)
    Array.from(filterContainer.children).forEach(function (tag) {
      if (!tag.id) { // 필터 레이블과 초기화 버튼 제외
        filterContainer.removeChild(tag);
      }
    });
    subscriptionSlider.noUiSlider.set([10000, 100000]); // 슬라이더 초기화
