...
<div class="login-container">
    ...
    <form action="pages/login/customer_login.php" method="POST">
...
<div class="login-container">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($row && password_verify($password, $row['PW_HASH']))  {
            ...
            $response = ['status' => 'success', 'message' => '로그인 되었습니다. 환영합니다!'];
            echo "<script>alert('" . $response['message'] . "');</script>";
            echo "<script>location.href='customer_login.php';</script>";
            exit;
            } else {
                $response = ['status' => 'error', 'message' => 'ID 또는 비밀번호가 잘못되었습니다.'];
                echo "<script>alert('" . $response['message'] . "');</script>";
                echo "<script>location.href='customer_login.php';</script>";
            }
