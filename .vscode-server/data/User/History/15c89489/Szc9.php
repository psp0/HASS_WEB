...
// Initialize query with basic selection
...
if (!empty($_POST['filter_type'])) {
    $types = implode("', '", array_map('htmlspecialchars', $_POST['filter_type']));
    $query .= " AND S.FILTER_TYPE IN ('$types')";
}
if (!empty($_POST['pm_sensor'])) {
    $sensors = implode("', '", array_map('htmlspecialchars', $_POST['pm_sensor']));
    $query .= " AND S.PM_SENSOR IN ('$sensors')";
}
...
while ($row = oci_fetch_array($stmt, OCI_ASSOC + OCI_RETURN_NULLS)) {
    ...
    foreach ($row as $column => $value) {
        echo "<div class='data-field'><span class='label'>" . htmlspecialchars($column) . ":</span> <span class='value'>" . htmlspecialchars($value) . "</span></div>";
    }
    echo "</div>";
}
echo "</div>";
