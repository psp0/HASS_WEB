...
<div class="main-container">
  <div class="filter-panel">
    <form method="GET" action="">
      ...
      <div class="accordion">화면 크기</div>
      <div class="panel" style="max-height: 1000px;">
        <div class="checkbox-group">
          <label><input type="checkbox" name="screen_size_inch[]" value="over75" <?php if (isset($_GET['screen_size_inch']) && in_array('over75', $_GET['screen_size_inch'])) echo 'checked'; ?>>75인치 초과</label>
          <label><input type="checkbox" name="screen_size_inch[]" value="65-75" <?php if (isset($_GET['screen_size_inch']) && in_array('65-75', $_GET['screen_size_inch'])) echo 'checked'; ?>>65인치 이상 - 75인치 이하</label>
          <label><input type="checkbox" name="screen_size_inch[]" value="55-65" <?php if (isset($_GET['screen_size_inch']) && in_array('55-65', $_GET['screen_size_inch'])) echo 'checked'; ?>>55인치 이상 - 65인치 미만</label>
          <label><input type="checkbox" name="screen_size_inch[]" value="43-55" <?php if (isset($_GET['screen_size_inch']) && in_array('43-55', $_GET['screen_size_inch'])) echo 'checked'; ?>>43인치 이상 - 55인치 미만</label>
          <label><input type="checkbox" name="screen_size_inch[]" value="under43" <?php if (isset($_GET['screen_size_inch']) && in_array('under43', $_GET['screen_size_inch'])) echo 'checked'; ?>>43인치 미만</label>
        </div>
      </div>


      <div class="accordion">설치 유형</div>
      <div class="panel" style="max-height: 1000px;">
        <div class="checkbox-group">
          <label><input type="checkbox" name="install_type[]" value="벽걸이" <?php if (isset($_GET['install_type']) && in_array('벽걸이', $_GET['install_type'])) echo 'checked'; ?>>벽걸이</label>
          <label><input type="checkbox" name="install_type[]" value="스탠드형" <?php if (isset($_GET['install_type']) && in_array('스탠드형', $_GET['install_type'])) echo 'checked'; ?>>스탠드형</label>
        </div>
      </div>


      <div class="accordion">디스플레이</div>
      <div class="panel" style="max-height: 1000px;">
        <div class="checkbox-group">
          <label><input type="checkbox" name="display_type[]" value="OLED" <?php if (isset($_GET['display_type']) && in_array('OLED', $_GET['display_type'])) echo 'checked'; ?>>OLED</label>
          <label><input type="checkbox" name="display_type[]" value="LCD" <?php if (isset($_GET['display_type']) && in_array('LCD', $_GET['display_type'])) echo 'checked'; ?>>LCD</label>
        </div>
      </div>


      <div class="accordion">해상도</div>
      <div class="panel" style="max-height: 1000px;">
        <div class="checkbox-group">
          <label><input type="checkbox" name="resolution[]" value="8K" <?php if (isset($_GET['resolution']) && in_array('8K', $_GET['resolution'])) echo 'checked'; ?>>8K</label>
          <label><input type="checkbox" name="resolution[]" value="4K" <?php if (isset($_GET['resolution']) && in_array('4K', $_GET['resolution'])) echo 'checked'; ?>>4K</label>
          <label><input type="checkbox" name="resolution[]" value="QHD" <?php if (isset($_GET['resolution']) && in_array('QHD', $_GET['resolution'])) echo 'checked'; ?>>QHD</label>
          <label><input type="checkbox" name="resolution[]" value="FHD" <?php if (isset($_GET['resolution']) && in_array('FHD', $_GET['resolution'])) echo 'checked'; ?>>FHD</label>
          <label><input type="checkbox" name="resolution[]" value="HD" <?php if (isset($_GET['resolution']) && in_array('HD', $_GET['resolution'])) echo 'checked'; ?>>HD</label>
        </div>
      </div>
  ...
  <div class="content-panel">
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="filter-tags">
        ...
        displayFilterTags('screen_size_inch', $_GET['screen_size_inch'] ?? [], [
          'over75' => '75인치 초과',
          '65-75' => '65인치 이상 - 75인치 이하',
          '55-65' => '55인치 이상 - 65인치 미만',
          '43-55' => '43인치 이상 - 55인치 미만',
          'under43' => '43인치 미만'
        ]);
        ...
        displayFilterTags('display_type', $_GET['display_type'] ?? [], [
          'OLED' => 'OLED',
          'LCD' => 'LCD'
        ]);

        displayFilterTags('resolution', $_GET['resolution'] ?? [], [
          '8K' => '8K',
          '4K' => '4K',
          'QHD' => 'QHD',
          'FHD' => 'FHD',
    ...
    <div class="model-container" id="model-container">
      <div class="data-container">
        ...
        $rangeQuery = "SELECT MIN(YEARLY_FEE) AS MIN_FEE , MAX(YEARLY_FEE) AS MAX_FEE FROM MODEL WHERE MODEL_TYPE ='TV'";
       ...
       GROUP BY p.MODEL_ID";
        ...
        $query = "SELECT mts.MODEL_ID, m.MODEL_NAME, m.MANUFACTURER, mts.SCREEN_SIZE_INCH, mts.INSTALL_TYPE, 
                  mts.DISPLAY_TYPE, mts.RESOLUTION, m.YEARLY_FEE, m.COLOR, m.ENERGY_RATING, m.RELEASE_YEAR,
                  NVL(AVG(mr.RATING), 0) AS AVG_RATING, COUNT(mr.RATING) AS RATING_COUNT
                  FROM MODEL_TV_SPEC mts
                  JOIN MODEL m ON mts.MODEL_ID = m.MODEL_ID
                  LEFT JOIN MODEL_RATING mr ON mts.MODEL_ID = mr.MODEL_ID";

        $conditions = [];
        $params = [];

        if (!empty($_GET['screen_size_inch'])) {
          $screenConditions = [];
          foreach ($_GET['screen_size_inch'] as $size) {
            switch ($size) {
              case 'over75':
                $screenConditions[] = "mts.SCREEN_SIZE_INCH > 75";
                break;
              case '65-75':
                $screenConditions[] = "mts.SCREEN_SIZE_INCH >= 65 AND mts.SCREEN_SIZE_INCH <= 75";
                break;
              case '55-65':
                $screenConditions[] = "mts.SCREEN_SIZE_INCH >= 55 AND mts.SCREEN_SIZE_INCH < 65";
                break;
              case '43-55':
                $screenConditions[] = "mts.SCREEN_SIZE_INCH >= 43 AND mts.SCREEN_SIZE_INCH < 55";
                break;
              case 'under43':
                $screenConditions[] = "mts.SCREEN_SIZE_INCH < 43";
                break;
            }
          }
          $conditions[] = '(' . implode(' OR ', $screenConditions) . ')';
        }

        if (!empty($_GET['install_type'])) {
          $install_types = $_GET['install_type'];
          $conditions[] = "mts.INSTALL_TYPE IN ('" . implode("','", array_map('htmlspecialchars', $install_types)) . "')";
        }

        if (!empty($_GET['display_type'])) {
          $display_types = $_GET['display_type'];
          $conditions[] = "mts.DISPLAY_TYPE IN ('" . implode("','", array_map('htmlspecialchars', $display_types)) . "')";
        }

        if (!empty($_GET['resolution'])) {
          $resoulutions = $_GET['resolution'];
          $conditions[] = "mts.RESOLUTION IN ('" . implode("','", array_map('htmlspecialchars', $resoulutions)) . "')";
        }
        ...
        $query .= "
        GROUP BY mts.MODEL_ID, m.MODEL_NAME, m.RELEASE_YEAR, m.MANUFACTURER, mts.SCREEN_SIZE_INCH,
          mts.INSTALL_TYPE, mts.DISPLAY_TYPE, mts.RESOLUTION, m.YEARLY_FEE, m.COLOR, m.ENERGY_RATING
        ORDER BY 
          CASE 
              WHEN (SELECT COUNT(*) FROM PRODUCT p WHERE p.MODEL_ID = mts.MODEL_ID AND p.PRODUCT_STATUS = '재고') > 0 THEN 0
              ELSE 1
          END ASC, 
        ...
        while ($row = oci_fetch_array($stmt, OCI_ASSOC + OCI_RETURN_NULLS)) {
          ...
          echo "<li>화면 크기: <span style='color: #000; font-weight: 550;'>" . htmlspecialchars($row['SCREEN_SIZE_INCH']) . "인치</span> · 설치 유형: <span style='color: #000; font-weight: 550;'>" . htmlspecialchars($row['INSTALL_TYPE']) . "</span></li>";
          echo "<li>디스플레이: <span style='color: #000; font-weight: 550;'>" . htmlspecialchars($row['DISPLAY_TYPE']) . "</span> · 해상도: <span style='color: #000; font-weight: 550;'>" . htmlspecialchars($row['RESOLUTION']) . "</span></li>";
